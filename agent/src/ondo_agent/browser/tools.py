...
def _deny_origin(ctx: ToolContext, url: str, reason: str) -> None:
    ...
    raise PermissionDenied(f"{url} is not an allowed origin for this agent ({reason.replace('_', ' ')})",
                           kind="browser", reason=reason, target=url)
...
async def _gate_submit(ctx: ToolContext, s: BrowserSession, *, tool: str, args: dict[str, Any],
                       element: str, how: str) -> tuple[bool, str]:
    ...
    values = [ApprovalValue(k, v, before.get(k) if before.get(k) != v else None) for k, v in now.items()
              if v != before.get(k)] or [ApprovalValue(k, v) for k, v in list(now.items())[:12]]
    action = ProposedAction(tool=tool, arguments=args, description=f"{how} {element} on {title}",
                            max_effect="submit", url=url, element=element)
...
async def fill_form(args: dict[str, Any], ctx: ToolContext) -> ToolResult:
    s = _session(ctx)
    fields = [{s.ref_key: f["ref"], "name": f["name"], "type": f.get("type", "textbox"), "value": str(f["value"])}
...
async def press_key(args: dict[str, Any], ctx: ToolContext) -> ToolResult:
    ...
    if key.lower() in ("enter", "return", "numpadenter"):
        ok, msg = await _gate_submit(ctx, s, tool="browser_press_key", args=args, element="the focused field",
                                     how="Press Enter in")
        if not ok:
            return ToolResult(msg, detail={"summary": "Not approved", "approval": "refused"})
...
_ELEMENT = {"type": "string", "description": "What the element is, in words, e.g. 'Submit button'. Shown in the step log."}
...
def browser_tools() -> list[ToolSpec]:
    ...
    return [
        ToolSpec("browser_navigate",
                 ...
                 obj({"url": {"type": "string"}}, ["url"]), navigate, max_effect="read",
                 title=lambda a: f"Opened {a['url']}", **common),
        ToolSpec("browser_snapshot",
                 ...
                 obj({}), snapshot, max_effect="read", title=lambda a: "Read the page", **common),
        ToolSpec("browser_click",
                 ...
                 obj({"ref": _REF, "element": _ELEMENT}, ["ref"]), click, max_effect="submit",
                 title=lambda a: f"Clicked {a.get('element') or a['ref']}", **common),
        ToolSpec("browser_type",
                 ...
                 obj({"ref": _REF, "element": _ELEMENT, "text": {"type": "string"},
                      "submit": {"type": "boolean", "description": "Press Enter after typing. Default false."}},
                     ["ref", "text"]), type_text, max_effect="submit",
                 title=lambda a: f"Typed into {a.get('element') or a['ref']}", **common),
        ToolSpec("browser_fill_form",
                 ...
                 obj({"fields": {"type": "array", "items": obj({
                     "ref": _REF, "name": {"type": "string", "description": "The field's label."},
                     "type": {"type": "string", "enum": ["textbox", "checkbox", "radio", "combobox", "slider"]},
                     "value": {"type": "string"}}, ["ref", "name", "type", "value"])}}, ["fields"]),
                 fill_form, max_effect="read", title=lambda a: f"Filled {len(a['fields'])} fields", **common),
        ToolSpec("browser_select_option", "Choose one or more options in a dropdown by ref.",
                 obj({"ref": _REF, "element": _ELEMENT, "values": {"type": "array", "items": {"type": "string"}}},
                     ["ref", "values"]), select_option, max_effect="read",
                 title=lambda a: f"Selected {', '.join(a['values'])}", **common),
        ToolSpec("browser_press_key",
                 "Press a key such as Tab, Escape or ArrowDown. Enter may submit a form, so it stops for approval.",
                 obj({"key": {"type": "string"}}, ["key"]), press_key, max_effect="submit",
                 title=lambda a: f"Pressed {a['key']}", **common),
        ToolSpec("browser_navigate_back", "Go back to the previous page.", obj({}), navigate_back,
                 max_effect="read", title=lambda a: "Went back", **common),
        ToolSpec("browser_wait_for", "Wait for text to appear or disappear, or for a number of seconds (at most 30).",
                 obj({"text": {"type": "string"}, "textGone": {"type": "string"},
                      "time": {"type": "number", "maximum": 30}}), wait_for, max_effect="read",
                 title=lambda a: "Waited for the page", **common),
    ]
